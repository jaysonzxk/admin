  ...
  <d2-container>
    ...
    <!-- <div class="widgets" ref="widgets">
      <div :class="['widgets-wrapper', customizing ? 'widgets-wrapper-bg' : '']">
        ...
        <grid-layout ref="gridlayout" :layout.sync="layout" :col-num="colNum" :row-height="1" :is-draggable="customizing"
          :vertical-compact="false" :use-css-transforms="true" :autoSize="true">
          <grid-item v-for="(item, index) in layout" :static="item.static" :x="item.x" :y="item.y" :w="item.w" :h="item.h"
            :i="item.i" :key="index" :isResizable="customizing" @container-resized="containerResizedEvent">
            <div v-if="customizing" class="customize-overlay">
              <el-button v-if="item.config && Object.keys(item.config).length !== 0" class="close" style="right: 60px;"
                type="primary" plain icon="el-icon-setting" size="small" @click="clickConfig(item, index)"
                circle></el-button>
              <el-button class="close" type="danger" plain icon="el-icon-close" size="small" @click="remove(index)"
                circle></el-button>
              <label>
            ...
            <component :class="customizing ? 'set-component-bg' : ''" :is="allComps[item.element]" :config="item.config || {}"
              :width="item.w" :height="item.h" :pxData="pxData[item.i]"></component>
          </grid-item>
        </grid-layout>
      </div>
    </div> -->
